...
func TestNewCommandTemplate(t *testing.T) {
	t.Run("can parse: cmd $arg1 $arg2 --parameter=default\\ value ", func(t *testing.T) {
		cmd := NewCommandTemplate("cmd $arg1 $arg2 --parameter=default\\ value")
		err := cmd.Parse()
		assert.Nil(t, err)
	...
	t.Run("can parse: cmd ...$wildcard-arg --parameter[P]=default\\ value ", func(t *testing.T) {
		cmd := NewCommandTemplate("cmd ...$wildcard-arg --parameter[P]=default\\ value")
		err := cmd.Parse()
		assert.Nil(t, err)
		assert.Equal(t, "cmd", cmd.Command())
	...
	t.Run("can parse: cmd $arg --flag1[f] --flag2[F] --parameter=", func(t *testing.T) {
		cmd := NewCommandTemplate("cmd $arg --flag1[f] --flag2[F] --parameter=")
		err := cmd.Parse()
